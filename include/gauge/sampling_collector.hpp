...
private:
    ...
    /*! Callbacks. */
    std::forward_list<CallbackInterface>
        collect_callbacks;
    /*! Python callbacks. */
    std::forward_list<py::object>
        py_collect_callbacks;
    /*! Time workers' loops going to sleep each iteration. */
    static constexpr auto sleep_interval = std::chrono::microseconds(
        1000
    );
    /*! Time workers' loops gonna sleep each iteration during pause. */
    static constexpr auto pause_sleep_interval = std::chrono::milliseconds(
        50
    );
