...
public:
     ...
     */
    static DstTimePointType convert_time_point(
    ...
    static DstTimePointType convert_time_point(
        const SrcTimePointType &time_point,
        const SrcDurationType   tolerance = std::chrono::nanoseconds{100},
        const int               limit     = 10) {
        auto const measurements = get_base_measurements(tolerance, limit);
     ...
     */
    static BaseMeasurements get_base_measurements(
        const SrcDurationType tolerance = std::chrono::nanoseconds{100},
        const int             limit     = 10) {
        auto iter_count = 0;
        ...
        do {
            ...
            const auto delta = SrcDurationType{std::abs(src_diff.count())};
            if (delta < epsilon) {
...
private:
    ...
    static constexpr DurationType max_duration() noexcept {
