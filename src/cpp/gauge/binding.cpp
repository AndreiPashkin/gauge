...
PYBIND11_MODULE(_gauge, m) {
    ...
    >(m,"Spans");
    // Exceptions.
    auto gauge_error = py::register_exception<GaugeError>(
        m,
        "GaugeError"
    );
    ...
    py::class_<TraceSample, std::shared_ptr<TraceSample>>(m, "TraceSample")
        ...
        .def_readwrite("monotonic_clock_timestamp",
    ...
    PySpan
        .def_readwrite("lifetime", &Span::lifetime)
    ...
    py::class_<SpanAggregator>(m, "SpanAggregator")
        ...
        .def(
            "subscribe",
            (void (SpanAggregator::*)(py::object))
                &SpanAggregator::subscribe
        )
        .def(
            "finish_open_spans",
                &SpanAggregator::finish_open_spans
        )
        .def(
            "__call__",
            &SpanAggregator::operator(),
            py::is_operator()
        )
        ;
    m.def(
