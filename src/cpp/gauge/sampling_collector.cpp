...
SamplingCollector::SamplingCollector(
    ...
    clocks_base_measurements{TimePointConversionUtil::get_base_measurements()},
...
bool SamplingCollector::is_stopped() const {
    return is_stopped_flag;
}
...
void SamplingCollector::register_own_thread() {
    unsigned long long own_thread_id;
    {
        ...
        own_thread_id = py::cast<unsigned long long>(py::module::import(
            "threading"
        ).attr("get_ident")());
    }
...
void SamplingCollector::clear_own_threads() {
    own_thread_ids.clear();
}
...
std::chrono::steady_clock::duration SamplingCollector::
...
//            timer.start("collect_frames");
            auto result = collect_frames(
                current_timestamp,
                frames_buffer
            );
//            timer.stop("collect_frames");
//            if ((std::chrono::steady_clock::now() - t1) >= profile_interval) {
//                SPDLOG_LOGGER_INFO(
//                    logger,
//                    "collect_frames -> {:f}",
...
#ifndef NDEBUG
            {
                ...
                for(auto const& item : *traces) {
                    if (prev_trace != nullptr) {
                        assert(prev_trace->monotonic_clock_timestamp <=
...
SamplingCollector::RawFrame::RawFrame(RawFrame &&raw_frame) noexcept :
    ...
    is_coroutine{raw_frame.is_coroutine},
    is_generator{raw_frame.is_generator},
    is_bottommost{raw_frame.is_bottommost},
    is_topmost{raw_frame.is_topmost},
    cookie{raw_frame.cookie},
    thread_id{raw_frame.thread_id},
...
Frame * SamplingCollector::construct_frame(
    ...
    frame->symbolic_name = detail::safe_encode(
        raw_frame.frame->f_code->co_name
    );
    frame->file_name = detail::safe_encode(
        raw_frame.frame->f_code->co_filename
    );
...
TraceSample* SamplingCollector::construct_trace(
    ...
    {
        detail::GILGuard gil_guard;
        trace_sample->thread_id = py::cast<unsigned long long>(
            raw_frames[0]->thread_id
        );
    }
...
bool SamplingCollector::is_paused() {
